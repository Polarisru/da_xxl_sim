...
/** \brief Disable port pins (set direction to input)
 *
 * \param [in] port Port number
 * \param [in] pins Pins to clear
 * \return void
 *
 */
void GPIO_DisablePortPins(uint8_t port, uint32_t pins)
{
  PORT->Group[port].DIRCLR.reg = pins;
}
 ...
 *
 */
void GPIO_SetFunction(uint8_t port, uint8_t pin, enum gpio_func_mode mode)
{
  if (pin > GPIO_MAX_PIN)
  {
    return;
 ...
 *
 */
void GPIO_SetupPin(uint8_t port, uint8_t pin, enum gpio_func_mode mode, uint8_t dir, uint8_t level)
{
